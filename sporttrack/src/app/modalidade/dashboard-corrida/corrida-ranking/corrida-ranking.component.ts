...
import { CorridaRankingDataSource } from './corrida-ranking-datasource';
...
@Component({
  selector: 'corrida-ranking-table',
  templateUrl: './corrida-ranking.component.html',
  styleUrls: ['./corrida-ranking.component.css']
})
export class CorridaRankingComponent implements OnInit {
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;

  @Output() selecionado = new EventEmitter<boolean>();

  dataSource: CorridaRankingDataSource | null;
  ...
  ngOnInit() {
    this.variaveisService.getListaVariaveis(2).subscribe(response => {
  ...
  buildTableForVariable(variable: number) {
    ...
    this.dadosService.getListaDadosAtuais(variable).subscribe(response => {
      ...
      this.dataSource = new CorridaRankingDataSource(this.paginator, this.sort, this.rankData);
    });
  }
  ...
  recuperaPontos() {
    ...
    this.pontoService.getPontosParaEntidadesParaPontoAtual(2).subscribe(response => {
