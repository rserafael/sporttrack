...
@Component({
  selector: 'app-corrida-grafico',
  templateUrl: './corrida-grafico.component.html',
  styleUrls: ['./corrida-grafico.component.css']
})
export class CorridaGraficoComponent implements OnInit {
  ...
  ngAfterViewInit() {   
    ...
    this.sharedEntity.entidadeInformada$.subscribe(ent => {
      console.log('Recebida nova ENTIDADE em CORRIDA-GRAFICO: ' + ent.entidade.nome)
      console.log('Recebida nova VARIAVEL em CORRIDA-GRAFICO: ' + ent.variavel.nome)
       ...
       this.dadosService.getListaDadosEntidadeVariavel(this.entidade.id, this.variavel.id).subscribe(response => {
         ...
         this.chart = this.AmCharts.makeChart("serialdivcorrida", {
           "type": "serial",
           "theme": "light",
           "hideCredits":true,
           "marginRight": 0,
